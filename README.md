...
📂 Database Schema
|  |  |  | 
|  |  | student_id (PK)email (Unique)major | 
|  |  | course_id (PK)course_code (Unique)credits | 
|  |  | enrollment_id (PK)semesterenrollment_date | 
|  |  | score (0–100)weight (0.0–1.0)assessment_type | 
|  |  | status (Present/Absent/Late)attendance_date | 
...
Create a file named .env in the root folder (do not commit this file!). Add your Render credentials:
